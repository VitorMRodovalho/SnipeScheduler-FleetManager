...
// Load Teams webhook settings from system_settings (stored in DB, not config.php)
$teamsRows = $pdo->query("SELECT setting_key, setting_value FROM system_settings WHERE setting_key LIKE 'teams_%'")->fetchAll(PDO::FETCH_KEY_PAIR);
$teamsEnabled     = ($teamsRows['teams_webhook_enabled']       ?? '0') === '1';
$teamsUrlFleetOps = $teamsRows['teams_webhook_url_fleet_ops']  ?? '';
$teamsUrlAdmin    = $teamsRows['teams_webhook_url_admin']      ?? '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'test_db') {
        ...
        if (!empty($newFleetOps) && !filter_var($newFleetOps, FILTER_VALIDATE_URL)) {
            $errors[] = 'Fleet Ops webhook URL is not a valid URL.';
            $urlError = true;
        }
        if (!empty($newAdmin) && !filter_var($newAdmin, FILTER_VALIDATE_URL)) {
            $errors[] = 'Admin webhook URL is not a valid URL.';
            $urlError = true;
        }
        ...
        if (!$urlError) {
            ...
            // Only update URL if a value was submitted (blank = keep existing)
            if ($newFleetOps !== '') $stmt->execute(['teams_webhook_url_fleet_ops', $newFleetOps]);
            if ($newAdmin     !== '') $stmt->execute(['teams_webhook_url_admin',     $newAdmin]);
            $messages[] = 'Teams configuration saved.';
            // Reload
            $teamsEnabled     = $newEnabled === '1';
            if ($newFleetOps !== '') $teamsUrlFleetOps = $newFleetOps;
...
<head>
    ...
    <link rel="stylesheet" href="assets/style.css?v=1.4.1">
...
<form method="post" action="<?= h($active) ?>" class="row g-3 settings-form" id="settings-form">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title mb-1" style="color:#6264a7;">
                            <i class="bi bi-microsoft-teams me-2"></i>Microsoft Teams Integration
                        </h5>
                        <p class="text-muted small mb-3">
                            Deliver notification cards to Teams channels via Power Automate HTTP trigger or Incoming Webhook URLs.
                            Webhook URLs contain authentication tokens — treat them as passwords.
                            Fleet Admins can enable/disable notifications and select per-event channels on the
                            <a href="notifications">Notifications</a> page, but cannot view these URLs.
                        </p>

                        <div class="alert alert-light border small mb-3">
                            <strong>How to get a webhook URL (Power Automate):</strong><br>
                            1. Open <a href="https://make.powerautomate.com" target="_blank" rel="noopener">Power Automate</a>
                            and create a new <strong>Instant cloud flow</strong>.<br>
                            2. Add trigger: <strong>When an HTTP request is received</strong> — no schema needed.<br>
                            3. Add action: <strong>Post adaptive card in a chat or channel</strong> → choose your Teams channel.<br>
                            4. In the card body field enter: <code>@{triggerBody()?['attachments'][0]['content']}</code><br>
                            5. Save the flow and copy the <strong>HTTP POST URL</strong> — paste it below.<br>
                            <span class="text-muted">Each channel requires its own flow and its own URL. The URL contains a
                            ...
                            <div class="row g-3 mb-3">
                                <div class="col-12">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox"
                                               name="teams_webhook_enabled" id="teams_webhook_enabled"
                                               value="1" <?= $teamsEnabled ? 'checked' : '' ?>>
                                        <label class="form-check-label fw-semibold" for="teams_webhook_enabled">
                                            Enable Teams Notifications
                                        </label>
                                    </div>
                                    <div class="form-text">
                                        When enabled, events configured for "Teams" or "Both" on the Notifications page
                                        will deliver cards to the channels below.
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-text mb-1">
                                        Receives: checkout, check-in, reservations, reminders, overdue alerts.
                                    </div>
                                    <div class="input-group">
                                        <input type="password"
                                               name="teams_webhook_url_fleet_ops"
                                               id="teams_url_fleet_ops"
                                               class="form-control font-monospace"
                                               placeholder="<?= $teamsUrlFleetOps ? 'Leave blank to keep existing URL' : 'https://prod-xx.eastus.logic.azure.com/...' ?>"
                                               autocomplete="off">
                                        <button type="button" class="btn btn-outline-secondary"
                                                onclick="toggleUrlReveal('teams_url_fleet_ops', this)">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                        <button type="button" class="btn btn-outline-primary"
                                                onclick="testTeamsFromSettings('fleet_ops')">
                                            Test
                                        </button>
                                    </div>
                                    <div id="teams-test-fleet_ops" class="mt-1 small"></div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-text mb-1">
                                        Receives: mileage anomalies, compliance expiry, admin-only alerts.
                                    </div>
                                    <div class="input-group">
                                        <input type="password"
                                               name="teams_webhook_url_admin"
                                               id="teams_url_admin"
                                               class="form-control font-monospace"
                                               placeholder="<?= $teamsUrlAdmin ? 'Leave blank to keep existing URL' : 'https://prod-xx.eastus.logic.azure.com/...' ?>"
                                               autocomplete="off">
                                        <button type="button" class="btn btn-outline-secondary"
                                                onclick="toggleUrlReveal('teams_url_admin', this)">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                        <button type="button" class="btn btn-outline-primary"
                                                onclick="testTeamsFromSettings('admin')">
                                            Test
                                        </button>
                                    </div>
                                    <div id="teams-test-admin" class="mt-1 small"></div>
                                </div>
                            </div>
...
<?php layout_footer(); ?>
<script>
function toggleUrlReveal(inputId, btn) {
...
function testTeamsFromSettings(audience) {
    var resultEl = document.getElementById('teams-test-' + audience);
